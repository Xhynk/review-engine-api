 ...
 */
fetch('https://'+ engine +'/reviews-api-v2/?user=test&key=cdff.1070bdaacf&threshold=4&query_v2=true&reviews_per_page='+ count)
.then(function(response){
	return response.json();
})
.then(function(json){
	let company = json.company[0],
	...
	if( data.aggregate != null && data.total_reviews != 0 ){
		...
		let header = document.createElement('h2');
		header.classList.add('rbd-header');
		header.innerHTML = 'Be the first to write a review: <a class="rbd-button rbd-small" target="_blank" rel="noopener nofollow" href="'+ reURL +'">Write a Review</a>';
